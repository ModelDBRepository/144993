% This file combines results of several "get_Prob_HHSTM.m" simulations
...
load(['HHSTM_Threshold&Average_rates_I_70_N_1e' num2str(log10(N)) '_N_s_1e6_dt_5e-3_#1.mat']);
...
for ii=2:L
load(['HHSTM_Threshold&Average_rates_I_70_N_1e' num2str(log10(N)) '_N_s_1e6_dt_5e-3_#' num2str(ii) '.mat']);
counter=ii-1;
...
p_s=zeros(L_I,2);
for ii=1:L_I
     temp= fit(s1_array(ii,:)',squeeze(AP_dist(:,:,ii)),ft_,fo_); %notice change from previous version: s1_array -> s1_array(ii,:)
     p_s(ii,1)=temp.a;
     p_s(ii,2)=temp.b;
end
save(['HHSTM_p(S)_dt_5e-3_N_s_1e6_N1e' num2str(log10(N)) '.mat'],'p_s','AP_dist','s1_array','I_array','N','dt');
