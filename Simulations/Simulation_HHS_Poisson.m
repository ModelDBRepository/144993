% Generates data file of 55 hours of Poisson stimulation of HHS model with
...
name=['Poisson_HHS_f=20_N=1e' num2str(log10(N),1) ]; %name of data output
...
for jj=1:L_I    
    for ii=1:L_f
       ...
       I0=I_array(jj);
       y0=[ -66.4379 0.0446 0.0446 0.0446  0.2959 0.2959 0.2959 0.2959 0.6451 s_initial]; %initial condition when s is approximatly at steady state
      
        y=y0; %intial conditions 

        dy2dt=zeros(1,10);  dy=dy2dt;
        alpha=zeros(1,4); %recovery rates
        ...
        for pp=1:stim_num
            ...
            for kk=1:current_cycle
                ...
                dy2dt(1)=(gNa.*(y(2).*y(3).*y(4).*y(9).*y(10)).*(VNa-y(1))+(y(5).*y(6).*y(7).*y(8)).*gK.*(VK-y(1))+gL.*(VL-y(1))+I)./Cm;% dVm/dt
                ...
                y=y+ dy2dt*dt+randn(1,10).*noise.*sqrt_dt_N;    
