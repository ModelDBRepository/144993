% Generates data file of 55 hours of stimulation of the HHMSIP model
...
name=['O2_HHMSIP_f=' num2str(f_array) '_alpha=' num2str(alpha_scaling) '_k=' num2str(k) '_N=1e' num2str(log10(N),1) ]; %name of data output
...
for jj=1:L_I    
    for ii=1:L_f
        ...
        if intermittent==1
            s_initial=0.88;%params(3,2)+0.001; %start just above threshold for intermittent mode
        else 
            s_initial=1;
        end
        
       I0=I_array(jj);
       y0=[ -66.4379 0.0446 0.0446 0.0446  0.2959 0.2959 0.2959 0.2959 0.6451 s_initial s_initial s_initial s_initial s_initial 0.89]; %initial condition when s is approximatly at steady state
      
        y=y0; %intial conditions 

        dy2dt=zeros(1,15);  dy=dy2dt;
        ...
        for pp=1:stim_num
            ...
            for kk=1:cycle
                ...
                dy2dt(1)=(gNa.*(y(2).*y(3).*y(4)).*y(9)*(w(1)*y(10)+w(2)*y(11)+w(3)*y(12)+w(4)*y(13)+w(5).*y(14)).*(VNa-y(1))+(y(5).*y(6).*y(7).*y(8)).*(gK+gM.*y(15)).*(VK-y(1))+gL.*(VL-y(1))+I)./Cm;% dVm/dt
                ...
                dy2dt(15)=(1-y(15)).*beta(5)-y(15).*alpha(5);  % ds6/dt  - pottasium inactivation  - notice the switch in alpha and beta!
                ...
                noise(15)=sqrt(abs(1-y(15)).*beta(5)+y(15).*alpha(5));  % ds6/dt  - pottasium inactivation  - notice the switch in alpha and beta!
...
%                     I0=8.3;
%                 end
                y=y+ dy2dt*dt+randn(1,15).*noise.*sqrt_dt_N;    
