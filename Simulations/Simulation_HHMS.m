% Generates data file of 55 hours of stimulation of HHMS model
...
I_array=7.7;%[7.5 7.7 7.9 8.1 8.3];% [microamper]
...
name=['O1_LHHMS_I0=7.7_alpha=' num2str(alpha_scaling) '_k=' num2str(k) '_N=1e' num2str(log10(N),1) ]; %name of data output
...
for jj=1:L_I    
    for ii=1:L_f
        ...
        if intermittent==1
            s_initial=params(3,2)+0.001; %start just above threshold for intermittent mode
        else 
            s_initial=1;
        end
